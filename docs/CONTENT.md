...
- DEEP-NDT의 확정 콘텐츠, 자산 출처와 반응형 구성은 [DEEP-NDT 상세 기준](projects/deep-ndt.md)을 따른다. 사용자 승인 시안의 파란 강조색과 지표 카드는 해당 프로젝트 본문에 한정한다.

- 프로젝트 개요는 공통 `ProjectOverview`를 사용한다. 모바일 설명 아래 메타데이터 2열, 태블릿부터 설명과 메타데이터 좌우 배치로 구성한다. 주요 담당 업무처럼 긴 항목은 넓은 칸을 선택할 수 있다. (2026-09-19 DEEP-NDT 기준 통일)
