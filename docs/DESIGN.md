...
### Mobile navigation decision — 2026-09-04

- `48rem` 미만의 공통 Header에서는 데스크톱 링크를 `ham-menu.svg` 버튼으로 재구성한다.
- 메뉴는 화면 오른쪽에서 전체 화면 너비로 들어오며, 상단에는 공통 Header와 같은 브랜드 링크를 둔다. `Home`, `Projects`, `Contact`의 큰 링크 타이포그래피와 32px 간격으로 정보 위계를 만든다.
- 공통 Header는 모든 화면 크기와 페이지에서 프로젝트 카테고리 고정 바와 같은 반투명 흰 배경과 제한적인 배경 흐림을 사용한다. 배경 위에서도 일관된 대비를 유지하도록 전경색은 검은색을 사용한다.
- 인덱스 페이지의 Hero 구간에서는 장면을 가리지 않도록 Header 배경과 흐림을 제거하고, Hero를 벗어나면 공통 유리 배경을 복원한다.
- 패널 하단에는 Contact 섹션과 같은 이름, 직무, 이메일, 카카오톡 정보를 메뉴 링크의 절반 정도 크기로 제공한다.
- 열린 동안 배경 스크롤과 본문 상호작용을 막고, 닫기 버튼·배경 선택·`Escape` 키를 모두 닫기 경로로 제공한다.
- `prefers-reduced-motion: reduce`에서는 패널과 배경의 전환을 즉시 처리한다.
