...
```css
--page-gutter: 1.5rem;      /* mobile: 24px, 사용자 확정 2026-09-19 */
...
## 프로젝트 상세 제작 흐름 — 2026-09-19

- 노트북 이상 화면에서 주로 열람할 것이라는 가정에 따라, 큰 화면의 전체 시안을 Figma에서 먼저 구성한다. 콘텐츠 순서, 시각적 위계, 이미지와 설명의 관계를 이 단계에서 정리한다.
- 웹 구현 전에 모바일에서 재구성이 필요한 부분을 먼저 검토한다. 긴 제목과 강제 줄바꿈, 다열 카드와 도식, 작은 주석, 가로로 긴 제품 화면을 확인하고 처리 방향과 필요한 자산을 정한다. 해결 방향이 불명확한 핵심 영역만 모바일 시안으로 검토한다.
- 모바일 전체 페이지를 Figma로 복제하는 것은 필수가 아니다. 실제 웹을 보면서 줄바꿈, 간격, 그리드 정렬과 터치 동작을 사용자와 조정한다.
- 작은 이미지 주석은 읽을 수 있는 HTML 텍스트로 분리한다. 전체 맥락이 필요한 화면은 전체 이미지와 번호 표시를 유지하고, 필요한 기능만 확대 이미지로 제공하는 구성을 검토한다. 슬라이드나 확대 기능은 콘텐츠에 맞춰 선택하며 모든 이미지에 일괄 적용하지 않는다.
- 슬라이드를 사용할 때는 첫 이미지의 시작 여백과 마지막 이미지의 끝 여백, 화면 가장자리까지 스크롤되는 방식, 현재 위치 표시와 키보드 접근을 함께 확인한다.
- 시각 설계 순서와 CSS 작성 순서를 구분한다. 큰 화면을 먼저 설계하더라도 모바일 기본 스타일과 큰 화면 확장 규칙으로 구현한다.
- 디자인이 확정되면 코드 정리와 화면 재검수까지 완료한다. 프로젝트별 수치와 예외는 `docs/projects/`에 기록하고, 반복적으로 채택된 규칙만 공통 기준으로 올린다. DEEP-NDT의 색상·카드·간격을 다른 프로젝트의 기본값으로 자동 적용하지 않는다.

### 프로젝트 상세 공통 요소와 색상 — 2026-09-19 확정

- 사이트의 브랜드 레드는 공통 Header, Home, Contact 등에 유지한다. 상세 페이지의 섹션 태그와 강조는 개별 프로젝트 색상을 사용한다.
- 프로젝트 팔레트는 주색(`accent`), 작은 글자용 강조색(`accentText`), 옅은 배경(`accentSurface`), 주색 위 전경색(`onAccent`)으로 나눈다. 텍스트용 색은 가독성을 위해 주색과 다른 명도를 사용할 수 있다.
- DEEP-NDT는 기존 파란 팔레트를 유지한다. 다른 프로젝트의 팔레트는 시안 제작 시 확정하며, 미지정 공통 요소는 중립색 initial 값을 사용한다.
- 요약은 DEEP-NDT에서 검수한 레이아웃을 공통으로 사용한다. 모바일은 설명 아래 80px 간격으로 메타데이터 2열, 48rem 이상은 설명과 메타데이터 좌우 배치, 75rem 이상 메타데이터 3열이다.
- 공통화 대상은 요약, 섹션 태그·제목·설명, 이미지·캡션, 회고다. 모바일의 설명–시각 자료 80px, 섹션 간 160px 리듬도 재사용한다.
- 이미지 캡션의 우측 2열 배치와 그림자는 콘텐츠 의도에 따라 선택한다. 회고는 모바일 우측 3열에 구분선까지 함께 맞춘다.
- 도식, 지표 카드, 노트북 목업, AI 화면 슬라이드는 현재 프로젝트 전용으로 유지한다. 다음 프로젝트의 실제 반복을 확인한 뒤 추가 공통화한다.
