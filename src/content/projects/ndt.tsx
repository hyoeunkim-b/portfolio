...
function Picture({ name, alt, width = 1920, height = 1080, caption, className = "" }: { name: string; alt: string; width?: number; height?: number; caption?: string; className?: string }) {
  return <ProjectFigure src={`/images/projects/ndt/${name}.webp`} alt={alt} width={width} height={height} caption={caption} className={`${styles.figure} ${className}`} />;
}
...
export function NdtOverview() {
  return <ProjectOverview label="DEEP-NDT 프로젝트 개요" metadata={[
    { label: "기업/클라이언트", value: <>DEEP-AI</> },
    { label: "진행 기간", value: <>2024. 7. ~ 2026. 8.</> },
    { label: "역할", value: <>Product Designer<br />PM 겸임(10개월)</> },
    { label: "협업 팀 구성", value: <>PM 2명(본인 포함), PD 1명(본인),<br />FE 3명, BE 3명, ML 5명</> },
    { label: "주요 담당 업무", value: <>UX/UI 설계, 디자인 시스템 구축, 요구사항 수집 및 일정 관리, 기능·콘텐츠 기획, QA, 커뮤니케이션 지원</>, wide: true }
  ]}>
    <p>DEEP-NDT는 비파괴검사 과정에서 수집한 데이터를 AI로 분석해<br className={styles.desktopBreak} />{" "}대형 산업 설비를 안전하게 유지보수할 수 있도록 지원하는 소프트웨어입니다.</p><p>기술 가능성만 검증된 MVP를 현장에서 사용할 수 있는 제품으로 고도화하기 위해,<br className={styles.desktopBreak} />{" "}제품 구조와 사용 경험을 설계하고 사용자 교육·가이드와 고객 대응을 지원했습니다.</p>
  </ProjectOverview>;
}
  ...
  return <div className={styles.content}>
    ...
    <div className={`${styles.band} ${styles.gray}`}>
      <section className={styles.part}>
        <Heading className={styles.heading} label="도메인 이해" title="대형 산업 설비의 상태는 어떻게 확인할까?"><p>원전·정유화학 시설 등에서는 설비를 손상시키지 않고 결함이나 상태를 확인하는 비파괴검사로 설비를 점검합니다.</p></Heading>
      ...
      <section className={styles.part}>
        <Heading className={styles.heading} label="상황" title={<>고객이 선택할 이유가 필요했던{" "}<br className={styles.mobileBreak} /><span className={styles.productName}>DEEP-NDT</span></>}><p>합류 당시에는 데이터 업로드, AI 결함 평가, 간이 보고서 내보내기가 가능한 초기 제품(MVP)이 구현되어 있었습니다.</p><p>기술적 가능성은 확인했지만, 고객이 기존 제품에서 DEEP-NDT로 전환하고 비용을 지불할 만큼의 차별점은 부족했습니다.</p></Heading>
      ...
      <section className={styles.part}>
        <Heading className={styles.heading} label="목표와 핵심 과제" title={<>AI 비파괴검사 기술을 검사자의 실제 업무에 적용할 수 있고,<br />고객에게 판매할 수 있는 제품으로 고도화한다.</>}><p>고객사에 MVP를 제공하고, 실제 사용 과정에서 얻은 피드백을 바탕으로 기능을 개발·개선했습니다.</p><p>이 과정에서 마주한 문제를 돌아보면, 핵심 과제는 다음 두 가지로 정리됩니다.</p></Heading>
    ...
    <div className={`${styles.band} ${styles.caseOne}`}>
      <section className={styles.part}>
        <Heading className={styles.heading} label="개선 사례 1" title="역할에 맞는 제품으로 구조화"><p>기존 제품에서는 검사자와 관리자가 동일한 흐름을 사용했습니다.</p><p>검사자는 판독 화면까지 여러 단계를 거쳐야 했고, 관리자는 설비 관리에 필요한 정보를 충분히 얻기 어려웠습니다.</p><p>두 사용자의 업무와 요구를 구분하고, 검사자는 판독에 빠르게 접근하며 관리자는 설비 현황과 검사 결과를 확인할 수 있도록 제품 구조를 재설계했습니다.</p></Heading>
      ...
      <section className={`${styles.part} ${styles.result}`}>
        <Heading className={styles.heading} result label="결과" title="판독 화면 진입은 빠르게, 설비 현황은 한눈에"><p>검사자는 판독 화면까지 더 적은 클릭과 짧은 시간으로 진입할 수 있게 되었습니다.</p><p>관리자는 설비 현황과 검사 결과를 한곳에서 확인할 수 있으며, 인터뷰에서도 다음 검사 대상을 파악하기 편해졌다는 반응을 확인했습니다.</p></Heading>
    ...
    <div className={`${styles.band} ${styles.gray}`}>
      <section className={styles.part}>
        <Heading className={styles.heading} label="개선 사례 2" title="검사자의 최종 판단을 돕기 위한 AI 경험 설계"><p>검사자들은 AI가 자신의 역할을 대체하거나, 자신의 판독 능력이 AI와 직접 비교되는 상황에 부담을 느꼈습니다.</p><p>그래서 검사자가 필요에 따라 AI를 활용하고, 결과를 확인·수정한 뒤 최종 판단하도록 설계했습니다.</p><p>판독 결과를 기록하고 보고서로 연결해, AI의 도움이 업무를 마무리하는 과정까지 이어지도록 했습니다.</p></Heading>
...
<Picture className={styles.annotated} name="ai-annotated" width={1280} height={939} alt="AI 결함 목록을 별도 탭에서 확인하고, AI 판독 결과를 보거나 숨기며, 검사자가 판정한 결함을 직접 보고서에 추가하는 IRIS 뷰어" />
      ...
      <section className={`${styles.part} ${styles.result}`}>
        <Heading className={styles.heading} result label="결과" title="현장에서 활용되는 AI, 검사자가 체감한 판독 지원"><p>현장 사용에서 AI 평가 기능 활용률은 97%, 사용 후 설문에서 AI 기능 만족도는 5점 만점에 4.3점으로 나타났습니다.</p><p>검사자 인터뷰에서는 반복 판독의 피로가 줄고, 사용 경험을 통해 AI의 도움을 체감했다는 반응을 확인했습니다.</p></Heading>
    ...
    <div className={`${styles.band} ${styles.closingBand}`}>
      <section className={styles.part}>
        <Heading className={styles.heading} label="결과" title="기술 검증용 MVP에서, 국내외 현장 도입과 수익화까지"><p>2026년, DEEP-NDT는 국내외 원전 및 정유화학 기업에 실제 도입되어 수익화를 실현했습니다.</p><p>고객사의 사용 피드백을 반영한 반복 개선과 팀의 제품화 노력이 사업 성과로 이어졌습니다.</p></Heading>
